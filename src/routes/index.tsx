...
const FALLBACK_LIMITATIONS: { title: string; detail: string }[] = [
  {
    title: "Data quality unknown",
    detail: "This diagnostic can't see how clean or structured your internal data actually is.",
  },
  {
    title: "Organizational buy-in",
    detail: "Political will and cross-team alignment matter as much as the technology.",
  },
  {
    title: "Budget constraints",
    detail: "Suggested tools assume budget is available — actual constraints aren't known here.",
  },
  {
    title: "Change management capacity",
    detail: "Rolling out automation takes team bandwidth this diagnostic can't measure.",
  },
];
...
function buildRoadmap(workflows: Workflow[]) {
  const withTier = workflows.filter((wf) => wf.readinessTier);
  if (withTier.length === 0) return null;

  const quickWins = withTier
    .filter((wf) => wf.readinessTier === "Quick Win")
    .sort((a, b) => b.score - a.score);
  const structural = withTier
    .filter((wf) => wf.readinessTier === "Structural Play")
    .sort((a, b) => b.score - a.score);

  return {
    now: quickWins.slice(0, 2),
    next: quickWins.slice(2),
    later: structural,
  };
}
...
function Index() {
  ...
  const roadmap = result ? buildRoadmap(result.workflows) : null;
          ...
          <div className="mx-auto max-w-3xl px-6 pb-24">
            ...
            {result && (
              <div className="pt-16 sm:pt-24">
                ...
                <section
                  className="animate-rise mt-6 rounded-2xl border border-dashed border-border bg-secondary/20 p-5 sm:p-6"
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                        This diagnostic is generated by an LLM from the company name and
                        optional description you provided — there's no live lookup of your
                        website, financials, or tech stack. The score and workflow
                        suggestions are informed estimates based on patterns for similar
                ...
                {roadmap && (
                  <section
                    ...
                    style={{ animationDelay: `${380 + result.workflows.length * 90}ms` }}
                  >
                    <h3 className="font-display text-xl font-bold tracking-tight">
                      Roadmap
                    </h3>
                    <p className="mt-1 text-sm text-muted-foreground">
                      A suggested sequence — quick wins first, structural work once those
                      are underway.
                    </p>
                    <div className="mt-5 grid gap-4 sm:grid-cols-3">
                      {(
                        [
                          { label: "Now", hint: "Start immediately", items: roadmap.now },
                          {
                            label: "Next",
                            hint: "Once Now is underway",
                            items: roadmap.next,
                          },
                          {
                            label: "Later",
                            hint: "Needs groundwork first",
                            items: roadmap.later,
                          },
                        ] as const
                      ).map((col) => (
                          ...
                          <div className="mt-3 space-y-2">
                            {col.items.length === 0 ? (
                              ...
                              col.items.map((wf) => (
                                <div
                                  key={wf.name}
                                  className="rounded-lg bg-secondary/40 px-3 py-2 text-sm font-medium"
                                >
                                  {wf.name}
                                </div>
                              ))
